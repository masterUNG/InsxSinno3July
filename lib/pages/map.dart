...
class _MyMapState extends State<MyMap> {
  ...
  Future<Null> editAndRefresh() async {
    ...
    print('### จำนวนของ ขนาด SQLite ==>> $timeEdit');
      ...
      for (var item in insxModelForEdits) {
        try {
          await editDataInsx2(item).then((value) async {
            ...
            if (timeEdit == 0) {
              print('EditData ครบหมดแล้ว จะ Clear SQLite และ Update มันใหม่');
  ...
  Future<Null> editDataInsx2(InsxModel2 insxModel2) async {
    ...
    await Dio().get(url).then((value) {
      print(
          '### value ที่ได้จากการ editDataWhereInvoiceNo ==>> ${insxModel2.invoice_no}');
